...
\begin{proof}
  ...
  \begin{enumerate}
    ...
    \item both $x_i$ and $x_j$ are disjoint from $A$.
  \end{enumerate}
  Now consider the slice choice array $\mathcal{S}_i=\left[S_1^i,...,S_n^i\right]$ corresponding to the slice-choice graph $G_i$ and the slice choice array $\mathcal{S}_j=\left[S_1^j,...,S_n^j\right]$ corresponding to the slice-choice graph $G_j$.
  Consider $Q_i=\bigcup_{k\in 1..n}S_k^i$ and $Q_j=\bigcup_{k\in 1..n}S_k^j$. By definition, both $Q_i$ and $Q_j$ are quorums for all processes. %is a quorum of both $p_i$ and $Q_j$ is a quorum of $p_j$.
