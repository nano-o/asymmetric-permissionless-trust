...
To prove~\Cref{thm:existsFQSiff}, we first show in~\Cref{thm:canonical} that for every federated fail-prone system $\mathbb{F}$ satisfying $Q^3(\mathbb{F})$, there exists a canonical quorum system $\overline{\mathbb{F}}$ for $\mathbb{F}$. Second, we show in~\Cref{thm:Q3_must_hold} that if $\mathbb{Q}$ is a federated quorum system for $\mathbb{F}$, then $Q^3(\mathbb{F})$ must hold.
...
\begin{definition}[Canonical federated quorum system for $\mathbb{F}$]
  ...
  \begin{enumerate}
    ...
    \item there is a fail-compatible set $A$ such that, for every process $p_j\in Q\setminus A$, $p_j$ has a slice that is a subset of $Q$.\todo{that's not right; in a given execution, $A$ is fixed; we shouldn't consider all possible $A$s} %(i.e.\ there is a fault-prone set $F$ of $p_j$ such that $P_j\setminus F\subseteq Q$).
  \end{enumerate}
\end{definition}
...
%For now, we continue our exposition using~\Cref{def:canonical} as above.
