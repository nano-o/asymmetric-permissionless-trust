...
The assumptions of the processes are satisfied in an execution if and only if the set of processes that actually fail is a set $A$ such that, for every $p_j\in I$, there exists $F\in\mathcal{F}_j$ such that $P_j\cap A \subseteq F$; such a set $A$ is called fail-compatible with $\mathbb{F}$.%Thus we say that a set of processes $F$ is fail-compatible with $\mathbb{F}$ whenever for every $p_i$ and $p_j$, if $P_i\cap P_j\neq \emptyset$ then $F\cap P_i\cap P_j\subseteq \mathcal{F}_i^* \cap \mathcal{F}_j^*$.
...
\begin{itemize}
  ...
  \item [Availability] If $F$ is a fail-prone set over $\mathcal{P}$, then there exists a quorum $Q$ of $p_i$ that is disjoint from $F$.
...
\end{enumerate}
\end{definition}
Note that a set $S=P_i\setminus F$ where $F\in\mathcal{F}_i$ is a quorum slice of $p_i$ in the terminology of Mazières, and thus the canonical federated quorum system for $\mathbb{F}$ is isomorphic to a FBQS as defined by Mazières.
