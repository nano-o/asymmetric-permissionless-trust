...
\abstract{
  ...
  However, using a quorum system requires prior agreement on the notion of quorum, which is hardly achievable in a permissionless networks.
  % Even asymmetric quorums systems, where each process makes its own assumptions, require that every two processes make compatible assumptions, which again is hard to achieve in a permissionless system.
  In practice, the Stellar Network solves the problem of building a quorum system in a permissionless network. %, which can be used to solve consensus, in a permissionless network.
...
A common problem in a distributed system is to solve a task (e.g.\ consensus) given some assumption about the possible failures that may occur. For example, one may describe possible failures using fail-prone systems. A fail-prone system $\mathcal{F}=\left\{F_1,...,F_n\right\}$ is a set of fail-prone sets, where $F_i\subseteq \mathcal{P}$ for every $i$, $\mathcal{P}$ being the set of all processes. $\mathcal{F}$ denotes the assumption that, in a given execution, the set of processes that actually fail and collude to disrupt the system is exactly one of the $F_i$s. However, the processes do not know which one it is.
...
Cachin  and Tackman define the notion of asymmetric quorum system for an asymmetric fail-prone system, which enables solving distributed computing tasks such as read-write registers and broadcast as long as the processes' assumptions about failures are not violated.
Moreover, Cachin and Tackmann show that property $Q^3(\mathbb{F})$ generalizes to asymmetric quorum systems, that $Q^3(\mathbb{F})$ holds if and only if there exists an asymmetric quorum system for $\mathbb{F}$, and that an asymmetric fail-prone system determines a canonical asymmetric quorum system.
...
\todo{rework following two paragraphs}
