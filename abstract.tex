...
Thus, given a fail-prone system $\mathcal{F}$ satisfying $Q^3\left(\mathcal{F}\right)$, $\overline{\mathcal{F}}$ is a natural quorum system to use.
...
In this paper, we ask whether fail-prone systems are a useful model for permissionless systems.
The problem is that, in a permissionless system, processes do not generally know what the set of participants is, and each process has a potentially different, partial view of the system.
Thus, the fail-prone systems of Hirt and Maurer, in which every process makes the same assumptions about failures over the entire system, does not make sense in a permissionless system.

The main contribution of this paper is to propose the new notion of \emph{federated fail-prone systems} which generalizes the fail-prone systems of Hirt and Maurer and which is adapted to the permissionless setting.
In contrast with fail-prone systems, where processes all make the same failure assumptions about the entire system, in a federated fail-prone system, each process (a) make its own, personal failures assumptions and (b) makes failure assumptions only about a subset of the system.
