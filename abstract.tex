...
\begin{definition}[Canonical federated quorum system for $\mathbb{F}$]
  ...
  \begin{enumerate}
    ...
    \item there is a fail-compatible set $A$ such that, for every process $p_j\in Q\setminus A$, $p_j$ has a slice that is a subset of $Q$.%(i.e.\ there is a fault-prone set $F$ of $p_j$ such that $P_j\setminus F\subseteq Q$).
  \end{enumerate}
\end{definition}
...
Surprisingly, As we explain in \Cref{sec:practice}, this turns out not to be a problem in practice: there is a way for processes to compute at runtime, without knowing what the fail-compatible sets are, whether a set of received messages represents one of their quorums in $\overline{\mathbb{F}}$.
