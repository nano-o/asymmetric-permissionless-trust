...
\todo[inline]{There is an issue with a node always belonging to its slices. This cannot represent classic quorum systems because a process cannot have a quorum that does not include itself.}
...
Given $\mathbb{F}$, Cachin and Tackmann also define an asymmetric quorum system for $\mathbb{F}$, which enables solving distributed computing tasks such as read-write registers and broadcast as long as no process' assumptions about failures are not violated.
...
In this paper, we show that, similarly to symmetric and asymmetric quorum systems, a Federated Byzantine Quorum System can be understood in terms of a fail-prone system. This is our main contribution.

Our result sheds new light on the mathematical foundations of Federated Byzantine Quorum System model. We also hope that our result will help address the problem of porting existing quorum-based algorithms to the Stellar model, and that it will enable system designer to build on the Federated Byzantine Quorum System model to devise new permissionless systems.
...
\begin{definition}[Federated Quorum System for $\mathbb{F}$]
  ...
  \begin{itemize}
    ...
    \item[Consistency] If $Q_i$ is a quorum of $p_i$ and $Q_j$ is a quorum of $p_j$, if $\mathcal{S}_i$ is a slice choice such that $Q_i$ is closed under $\mathcal{S}_i$, if $\mathcal{S}_j$ is a slice choice such that $Q_j$ is closed under $\mathcal{S}_j$, and if $A$ is a fail-compatible set, then there is a path $x_i$ in the slice-choice graph $G_i$ corresponding to $\mathcal{S}_i$ and a path $x_j$ in the slice-choice graph $G_j$ corresponding to $\mathcal{S}_j$ such that
      \begin{itemize}
        \item $x_i$ starts at $p_i$ and $x_j$ starts at $p_j$,
        \item $x_i$ consists exclusively of processes in $Q_i\setminus A$,
        \item $x_j$ consists exclusively of processes in $Q_j\setminus A$,
        \item $x_i$ and $x_j$ intersect.
      \end{itemize}
  \end{itemize}
\end{definition}

Informally, the Consistency property states that not only must two quorums of two processes $p_i$ and $p_j$ intersect, but some process in the intersection must be reachable from $p_i$ and $p_j$ despite failures.

\begin{thm}
  There exists a federated quorum system for $\mathbb{F}$ if and only if $Q^3(\mathbb{F})$ holds.
\end{thm}

To prove the theorem, we first show that for every federated fail-prone system $\mathbb{F}$ satisfying $Q^3(\mathbb{F})$, there exists a canonibb quorum system $\overline{\mathbb{F}}$ for $\mathbb{F}$. Second, we show that if $\mathbb{Q}$ is a federated quorum system for $\mathbb{F}$, then $Q^3(\mathbb{F})$ must hold.
...
\end{claim}
\begin{proof}
  $\overline{\mathbb{F}}$ satisfies the Availability Property because for every process $p_i$ and every slice-choice graph $G$, by definition of $\overline{\mathbb{F}}$, the set of processes reachable from $p_i$ is a quorum.
  ...
  To show that $\overline{\mathbb{F}}$ satisfies the Consistency property, consider two processes $p_i$ and $p_j$, two quorums $Q_i$ of $p_i$ and $Q_j$ of $p_j$, a slice choice $\mathcal{S}_i$ such that $Q_i$ is closed under $\mathcal{S}_i$, a slice choice $\mathcal{S}_j$ such that $Q_j$ is closed under $\mathcal{S}_j$, and a fail-compatible set $A$. By Property $Q^3(\mathbb{F})$, we immediately obtain a path $x_i$ in the slice-choice graph $G_i$ corresponding to $\mathcal{S}_i$ and a path $x_j$ in the slice-choice graph $G_j$ corresponding to $\mathcal{S}_j$ such that
    ...
    For the same reason, $x_j$ consists exclusively of processes in $Q_j$.
\end{proof}
