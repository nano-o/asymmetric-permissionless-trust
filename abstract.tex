...
\mytodo{Why not give the definition of the quorum system up front and then show that it satisfies }
...
\begin{definition}[Property $Q^3(\mathbb{F})$]
  ...
  \end{itemize}
\end{definition}
\mytodo{Why is this definition useful?}
...
\begin{definition}[Federated Quorum System for $\mathbb{F}$]
  ...
  \begin{itemize}
    ...
    \item[Consistency] If $Q_i$ is a quorum of $p_i$ and $Q_j$ is a quorum of $p_j$, if $\mathcal{S}_i$ is a slice choice such that $Q_i$ is closed under $\mathcal{S}_i$\mytodo{why does it need to be closed?}, if $\mathcal{S}_j$ is a slice choice such that $Q_j$ is closed under $\mathcal{S}_j$, and if $A$ is a fail-compatible set, then there is a path $x_i$ in the slice-choice graph $G_i$ corresponding to $\mathcal{S}_i$ and a path $x_j$ in the slice-choice graph $G_j$ corresponding to $\mathcal{S}_j$ such that
      \begin{itemize}
        \item $x_i$ starts at $p_i$ and $x_j$ starts at $p_j$,
